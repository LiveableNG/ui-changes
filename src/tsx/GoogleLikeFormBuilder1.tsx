interface Condition {
    type: 'equals' | 'not_equals' | 'empty' | 'not_empty' | null;
...
interface Question {
    id: number;
    type: 'short' | 'long' | 'radio' | 'checkbox' | 'dropdown' | 'file' | 'date' | 'time';
    title: string;
    key: string;
    description: string;
    placeholder: string;
...
type ConditionType = NonNullable<Condition['type']>;
...
const GoogleLikeFormBuilder = () => {
    ...
    const getAllExistingKeys = (formData: FormData): string[] => {
        const keys: string[] = [];
        formData.sections.forEach(section => {
            section.questions.forEach(question => {
                if (question.key) {
    ...
    const updateQuestionTitle = (sectionId: string, questionId: number, newTitle: string): void => {
        const section = formData.sections.find(s => s.id === sectionId);
        if (!section) return;
    ...
    const toggleQuestionSetting = (sectionId: string, questionId: number, setting: keyof Question): void => {
        const section = formData.sections.find(s => s.id === sectionId);
        if (!section) return;
    ...
    const updateSectionNavigation = (
        sectionId: string,
        questionId: number,
        rules: Partial<SectionNavigation>
    ): void => {
        const section = formData.sections.find(s => s.id === sectionId);
        if (!section) return;
    ...
    const updateQuestionCondition = (
        sectionId: string,
        questionId: number,
        conditionType: keyof Question['conditions'],
    ...
    const duplicateQuestion = (sectionId: string, questionId: number): void => {
        const section = formData.sections.find(s => s.id === sectionId);
        if (!section) return;
    ...
    const deleteSection = (sectionId: string): void => {
        const sectionExists = formData.sections.some(s => s.id === sectionId);
                        ...
                        questions: section.questions.map(question => {
                            if (question.id === questionId) {
                                return {
                                    ...question,
                                    type: newType,
                                    options: ['radio', 'checkbox', 'dropdown'].includes(newType)
                                        ? ['Option 1']
                                        : []
                                };
                            }
                                                                                                ...
                                                                                                {['equals', 'not_equals'].includes(question.conditions.visible_if?.type || '') && (
                                                                                                    <>
                                                                                                        {(() => {
                                                                                                            ...
                                                                                                            const hasOptions = selectedQuestion &&
                                                                                                                    ...
                                                                                                                    <input
                                                                                                                        ...
                                                                                                                        onChange={(e) => {
                                                                                                                            updateQuestionCondition(
                                                                                                                                ...
                                                                                                                                'visible_if',
                                                                                                                                {
                                                                                                                                    type: question.conditions.required_if.type,
                                                                                                                                    questionId: question.conditions.required_if.questionId,
                                                                                                ...
                                                                                                {['equals', 'not_equals'].includes(question.conditions.visible_if?.type || '') && (
                                                                                                    <>
                                                                                                        {(() => {
                                                                                                            ...
                                                                                                            // Check if selectedQuestion exists and has options array
                                                                                                            const hasOptions = selectedQuestion &&
                                                                                                                Array.isArray(selectedQuestion.options) &&
                                                                                                                selectedQuestion.options.length > 0;
                                                                                                            ...
                                                                                                            return (
                                                                                                                <div className="flex-1">
                                                                                                                    <input
                                                                                                                        ...
                                                                                                                        onChange={(e) => {
                                                                                                                            if (question.conditions.visible_if) {
                                                                                                                                updateQuestionCondition(
                                                                                                                                    section.id,
                                                                                                                                    question.id,
                                                                                                                                    'visible_if',
                                                                                                                                    {
                                                                                                                                        type: question.conditions.visible_if.type,
                                                                                                                                        questionId: question.conditions.visible_if.questionId,
                                                                                                                                        value: e.target.value
                                                                                                                                    } as Condition
                                                                                                                                );
                                                                                                                            }
